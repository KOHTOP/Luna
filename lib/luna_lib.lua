...
local function sanitizeConfigName(name)
    local cleaned = tostring(name or ""):gsub("[^%w _%-]", "")
...
local function configPath(window, configName)
    return string.format("%s/%s.json", window.config.folder, configName)
end
...
function Window:SaveConfig(name)
    ...
    if not hasFS then
        self:Notify({
            title = "Конфиг",
            message = "Executor не поддерживает файлы",
            kind = "error",
        })
        return false
    end

    if hasList and not isfolder(self.config.folder) then
    ...
    local ok = pcall(function()
        writefile(configPath(self, configName), HttpService:JSONEncode(data))
    end)

    self:Notify({
        title = "Конфиг",
        message = ok and ("Сохранён: " .. configName) or "Ошибка сохранения",
...
function Window:LoadConfig(name)
    ...
    if not hasFS then
        self:Notify({
            title = "Конфиг",
            message = "Нет доступа к файлам",
            kind = "error",
        })
        return false
    end

    local path = configPath(self, configName)
    if typeof(isfile) == "function" and not isfile(path) then
        self:Notify({
            title = "Конфиг",
            message = "Файл не найден: " .. configName,
            kind = "error",
        })
        return false
    end
    ...
    for key, value in pairs(data) do
        local setter = self.setters[key]
        if setter then
            local applyOk = pcall(setter, value)
            if applyOk then
                applied += 1
            end
        end
    ...
    self:Notify({
        title = "Конфиг",
        message = string.format("Загружен: %s (%d)", configName, applied),
        kind = "success",
    })
    return true
end
...
function Window:DeleteConfig(name)
    ...
    local ok = pcall(delfile, configPath(self, configName))
    self:Notify({
        title = "Конфиг",
        message = ok and ("Удалён: " .. configName) or ("Ошибка удаления: " .. configName),
...
function Window:ListConfigs()
    ...
    local ok, files = pcall(listfiles, self.config.folder)
...
function Window:ResetConfig()
    for key, setter in pairs(self.setters) do
        if type(self.state[key]) == "boolean" then
            pcall(function()
                setter(false)
            end)
        end
...
function Section:Toggle(options)
    ...
    if flag then
        self.window.setters[flag] = function(value)
            setState(value, true)
            if callback then
                callback(state)
            end
...
function Section:Slider(options)
    ...
    if flag then
        self.window.setters[flag] = function(v)
            commit(v, true)
            if callback then
                callback(value)
            end
...
function Section:Dropdown(options)
    ...
    if flag then
        self.window.state[flag] = selected
    end
    local isOpen = false
...
function Section:Keybind(options)
    ...
    if flag then
        self.window.state[flag] = (typeof(bind) == "EnumItem" and bind.Name) or nil
    end
    ...
    if flag then
        self.window.setters[flag] = function(v)
            local resolved = resolveBind(v) or v
...
function Section:Input(options)
    ...
    if flag then
        self.window.state[flag] = box.Text
    end
